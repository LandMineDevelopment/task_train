...
def get_task_details(task_id: int, db_conf: dict) -> dict | None:
    ...
    rows = psql_query(
        ...
        f"  SELECT task.from_user_id, task.to_user_id, task.task, task.project_id, task.conversation_id, task.source_message_id, conversation.kind AS conversation_kind"
    ...
    while running:
        ...
        if scan_pending:
            ...
            for task_id_str, uid_str in rows:
                ...
                env["SOURCE_MESSAGE_ID"] = str(details["source_message_id"] or "")
