...
TASK_TRAIN_COMPOSE_PROJECT="$PROJECT" docker compose exec -T web python3 -c "from urllib.request import Request, urlopen; import json; assert json.load(urlopen('http://127.0.0.1:8000/api/health'))['status'] == 'ok'; conversations = json.load(urlopen('http://127.0.0.1:8000/api/conversations'))['conversations']; assert conversations; assert json.load(urlopen(f\"http://127.0.0.1:8000/api/conversations/{conversations[0]['id']}\"))['conversation']['id'] == conversations[0]['id']; request = Request('http://127.0.0.1:8000/api/conversations', data=b'{\"title\": \"Browser test\"}', headers={'Content-Type': 'application/json'}, method='POST'); created = json.load(urlopen(request)); request = Request(f\"http://127.0.0.1:8000/api/conversations/{created['conversation_id']}/messages\", data=b'{\"message\": \"Verify workflow dispatch.\"}', headers={'Content-Type': 'application/json'}, method='POST'); queued = json.load(urlopen(request)); assert queued['status'] == 'queued'; assert json.load(urlopen(f\"http://127.0.0.1:8000/api/conversations/{created['conversation_id']}\"))['conversation']['title'] == 'Browser test'; assert 'Task Train' in urlopen('http://127.0.0.1:8000/').read().decode(); print(queued['task_id'])" > /tmp/browser-task-id
