...
def test_conversation_context_is_ordered(db, sandbox):
    ...
    first_id = db.execute(
        "SELECT tagg.append_conversation_message(%s, %s, %s, 'first', 'user')",
        (conversation, sandbox["user_id"], conductor),
    ).fetchone()[0]
    second_id = db.execute(
        "SELECT tagg.append_conversation_message(%s, %s, %s, 'second', 'assistant')",
        (conversation, conductor, sandbox["user_id"]),
    ).fetchone()[0]
    messages = db.execute(
        "SELECT id, parent_id, seq_num FROM tagg.message "
        "WHERE conversation_id = %s ORDER BY seq_num",
        (conversation,),
    ).fetchall()
